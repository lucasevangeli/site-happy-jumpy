...
async function debugSpecificTicket(code) {
  console.log(`--- DIAGNÓSTICO DO INGRESSO ${code} ---`);
  const snapshot = await db.collection('tickets').where('code', '==', code).get();
  
  if (snapshot.empty) {
    console.log(`Ingresso ${code} não encontrado.`);
    return;
  }
...
debugSpecificTicket('CNK900').catch(console.error);
