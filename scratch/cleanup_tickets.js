const admin = require('firebase-admin');
const { getFirestore } = require('firebase-admin/firestore');

const privateKey = `-----BEGIN PRIVATE KEY-----\\n/sQ3bZHMA2YdkVfE4UK7\n-----END PRIVATE KEY-----\n`.replace(/\\n/g, '\n');

if (!admin.apps.length) {
  admin.initializeApp({
    credential: admin.credential.cert({
      projectId: "happy-jumpy",
      clientEmail: "firebase-adminsdk-fbsvc@happy-jumpy.iam.gserviceaccount.com",
      privateKey: privateKey,
    }),
    databaseURL: "https://happy-jumpy-default-rtdb.firebaseio.com",
  });
}

const db = getFirestore(admin.app(), 'happy');

async function deleteCollection(collectionPath) {
  const collectionRef = db.collection(collectionPath);
  const query = collectionRef.limit(100);

  return new Promise((resolve, reject) => {
    deleteQueryBatch(query, resolve).catch(reject);
  });
}

async function deleteQueryBatch(query, resolve) {
  const snapshot = await query.get();

  const batchSize = snapshot.size;
  if (batchSize === 0) {
    resolve();
    return;
  }

  const batch = db.batch();
  snapshot.docs.forEach((doc) => {
    batch.delete(doc.ref);
  });
  await batch.commit();

  process.nextTick(() => {
    deleteQueryBatch(query, resolve);
  });
}

console.log('Iniciando limpeza da coleção tickets...');
deleteCollection('tickets')
  .then(() => {
    console.log('Sucesso: Todos os ingressos foram removidos.');
    process.exit(0);
  })
  .catch((err) => {
    console.error('Erro ao deletar ingressos:', err);
    process.exit(1);
  });
